...
// ==================== VERIFICAÇÃO DE STATUS DO ADMIN ====================
function checkFirebaseAdminStatus() {
    if (window.currentUser && window.firebaseAdminEmails.includes(window.currentUser.email)) {
        console.log('👑 Usuário é administrador via Firebase');
        window.currentUser.isAdmin = true;
        window.currentUser.firebaseUser = true;
        
        // Atualizar no localStorage
        localStorage.setItem('reputai_user', JSON.stringify(window.currentUser));
        
        // Carregar funções administrativas se disponível
        setTimeout(() => {
            if (typeof loadAdminFunctions === 'function') {
                loadAdminFunctions();
...
window.checkFirebaseAdminStatus = checkFirebaseAdminStatus;
